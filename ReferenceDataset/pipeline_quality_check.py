...
# ---------------------------------------------------------------------------
DATASET_PATHS = {
    "reference": Path("/home/elenafalle/ReferenceDataset/all"),
}
...
def print_comparison(all_results: dict, run_dir: Path) -> None:
    ...
    lines = []
    lines.append("")
    lines.append(f"  {'Modell':<35} {'Samples':>10} {'WER':>10} {'WER %':>10}")
    lines.append("  " + "-" * 57)

    for key, r in all_results.items():
        lines.append(f"  {r['label']:<35} {r.get('num_samples', '?'):>10} {r['wer']:>10.4f} {r['wer_percent']:>9.2f}%")

...
from src.config import TRAIN_DIR, VAL_DIR, PARAKEET_HPARAMS, log
...
def _apply_lora(encoder: nn.Module, target_names: list[str],
    ...
    # Encoder-Basisgewichte einfrieren, nur LoRA-Params bleiben trainierbar
...
def _speed_perturb_manifest(
    ...
    """Erzeugt speed-perturbed Kopien aller Trainings-Samples und schreibt ein
    augmentiertes Manifest. Bereits existierende WAV-Dateien werden nicht neu erzeugt.

    Gibt die Gesamtanzahl der Manifest-Einträge zurück.
    """
    ...
    for entry in originals:
        for rate in speed_rates:
            ...
            if not out_path.exists():
                ...
                # resample_poly(x, up=10, down=round(10*rate)):
                # rate=1.1 → down=11 → fewer samples → plays faster
                # rate=0.9 → down=9  → more  samples → plays slower
...
def _merge_lora(encoder: nn.Module) -> None:
    """Merged LoRA-Adapter in die Basisgewichte zurück und stellt nn.Linear wieder her."""
    merged = 0
    for mod_name, module in list(encoder.named_modules()):
...
def evaluate(model_path_or_name: str, label: str, out_dir: Path) -> dict:
    ...
    # CUDA graphs are incompatible with PyTorch 2.10 / CUDA 13 (cudaStreamGetCaptureInfo
    # returns 5 values instead of 6). Disable them for inference.
    with open_dict(model.cfg.decoding):
        model.cfg.decoding.greedy.use_cuda_graph_decoder = False
    model.change_decoding_strategy(model.cfg.decoding, verbose=False)

    test_samples = load_metadata(cfg.TEST_DIR)
    ...
    # NeMo transcribe() return type varies by version
...
def finetune(run_dir: Path) -> None:
    ...
    # --- Generate NeMo manifests from CSV ---
    ...
    _csv_to_nemo_manifest(TRAIN_DIR, train_manifest)
    _csv_to_nemo_manifest(VAL_DIR, val_manifest)

    # Speed-Perturbation: 0.9×, 1.0×, 1.1× → 3× mehr Trainingsdaten
    ...
    with open_dict(asr_model.cfg):
        ...
        # 0.9× Speed → Samples bis ~12 s; max_duration auf 40 s hochsetzen
        asr_model.cfg.train_ds.max_duration = 40.0

        # Warmup = 10% der Gesamtsteps (per Expertenempfehlung für kleine Datasets).
        # max_steps direkt angeben: NeMo überspringt dann len(dataset) und
        # kann mit Lhotse-IterableDatasets umgehen.
    ...
    # Encoder einfrieren, aber LayerNorm-Schichten auftauen.
    # LayerNorm (γ, β) passt Feature-Verteilungen an neue akustische Domäne an,
    # ohne die gelernten Repräsentationen zu verändern (0.25M Params gesamt).
    # Vollständiges Einfrieren des Encoders verhindert laut NeMo-Doku jegliches Lernen.
